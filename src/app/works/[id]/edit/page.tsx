...
import type { WriterShare, PublisherShare, VersionType, Work, WriterCapacity, PublisherRole } from '@/types';
...
export default function EditWorkPage() {
  ...
  const { sidebarCollapsed, writers, publishers, works, updateWork, addToast } = useStore();
  ...
  useEffect(() => {
    if (work) {
      setTitle(work.title);
      setIswc(work.iswc || '');
      setVersionType(work.versionType);
      setLanguage(work.language || 'EN');
      setStatus(work.status);
      setWriterShares(work.writerShares || []);
      setPublisherShares(work.publisherShares || []);
    }
  }, [work]);
    ...
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2">Work Not Found</h1>
          <Button onClick={() => router.push('/works')}>Back to Works</Button>
        </div>
      </div>
    );
  }
  ...
  const handleSave = () => {
    ...
    addToast({ type: 'success', title: 'Work updated' });
    router.push('/works');
  };
      ...
      <main className="transition-all duration-300" style={{ marginLeft: sidebarCollapsed ? 72 : 256 }}>
        <div className="p-8 max-w-4xl">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <motion.h1 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-3xl font-bold text-zinc-900 dark:text-white">Edit Work</motion.h1>
              <p className="text-zinc-500">{work.workId}</p>
            </div>
          </div>
          ...
          <Card className="p-6 mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Select label="Status" options={[{ value: 'draft', label: 'Draft' }, { value: 'pending', label: 'Pending' }, { value: 'registered', label: 'Registered' }, { value: 'rejected', label: 'Rejected' }]} value={status} onChange={(e) => setStatus(e.target.value)} />
                  ...
                  <div key={share.id ?? idx} className="p-4 border border-zinc-200 dark:border-zinc-700 rounded-lg">
                    <div className="grid grid-cols-6 gap-3">
                      <div className="col-span-2">
                        ...
                        <Select label="Role" options={[{ value: 'E', label: 'Original Publisher' }, { value: 'AM', label: 'Administrator' }, { value: 'SE', label: 'Sub-Publisher' }]} value={share.role || 'E'} onChange={(e) => updatePublisherShare(idx, 'role', e.target.value as PublisherRole)} />
